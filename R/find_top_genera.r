...
#' @param top_number Numeric. Top number of taxa to include. Default = 50.
#' @param centrality_p Numeric. The probablity threahold for Pearson correlation. Default value = 0.05
#' @param centrality_r_upper Numeric. The upper level of Pearson correlation (r) to consider for centrality calculation. Include r above this value. Default = 0.7.
#' @param centrality_r_lower Numeric. The lower level of Pearson correlation (r) to consider for centrality calculation. Include r below this value. Default = 0
#'
#' @export

topGenera <- function(avt_dat, out_dir="avt_out", top_number=50, centrality_p=0.05, centrality_r_upper=0.7, centrality_r_lower=0){
	...
	for(grouping_idx in 1:length(sample_grouping)){
		...
		if(nrow(current_count) > 0){
			print(paste("Processing group:", current_grouping_name, sep=" ")) 
			...
			#print("Top abundance")
			top_genera[[paste("ABUN", current_grouping_name, sep="_")]] = topByAbundance(mean_genus_count = current_count_mean_genus, top_number = top_number)
			#print("Top prevalence")
			top_genera[[paste("PREV", current_grouping_name, sep="_")]] = topByPrevalence(mean_genus_count = current_count_mean_genus, top_number = top_number)
			#print("Top centrality")
			if(TRUE){
				top_genera[[paste("CENT", current_grouping_name, sep="_")]] = topByCentrality(mean_genus_count = current_count_mean_genus, 
																								taxa = taxa, 
																								out_dir = out_dir, 
			...
			print(paste("Insufficient data for grouping:", current_grouping_name, sep=" ")) 
		}
	}
...
topByAbundance <- function(mean_genus_count, top_number){
	mean_genus_count_long <- reshape2::melt(mean_genus_count, id.vars="Genus")
	colnames(mean_genus_count_long) <- c("Genus", "sample_id", "value")
	out <- plyr::ddply(mean_genus_count_long, "Genus", summarize, base_mean_genus=(mean(.data$value, na.rm = TRUE)))
...
topByCentrality <- function(mean_genus_count, taxa, out_dir, current_grouping_name, top_number, centrality_p, centrality_r_upper, centrality_r_lower){
	...
	centrality <- igraph::betweenness(correlation_graph)
	centrality <- as.data.frame(centrality)
	centrality$Genus <- row.names(centrality)
	row.names(centrality) <- NULL
	centrality <- dplyr::rename(centrality, "betweeness" = 'centrality')
	centrality <- dplyr::select(centrality, .data$Genus, .data$betweeness)
